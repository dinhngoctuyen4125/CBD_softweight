...
def eval_perturbation_ratio(eval_dataloader, perturb_dataloader, model):
    ...
    for batch, perturb_batch in tqdm(zip(eval_dataloader, perturb_dataloader)):
        ...
        with torch.no_grad():
            ...
            combined_outputs = model(**combined_batch)

        combined_logits = combined_outputs.logits
        gt_loss = get_batch_loss(combined_logits[:bsz], batch['labels'])
        perturb_loss = get_batch_loss(combined_logits[bsz:], perturb_batch['labels']).view(bsz, seq_len)
...
def eval_bleu(gen_outputs, ground_truths):
    ...
    if evaluate is None:
        raise ModuleNotFoundError(
            "Missing dependency `evaluate`. Install it or use `eval_rouge_recall` only."
        )
...
def eval_rouge_recall(gen_outputs, ground_truths):
    """
    Compute ROUGE-1/ROUGE-L recall without external deps.

    This is a lightweight approximation of `rouge_score` using whitespace tokenization.
    """
    ...
    def _lcs_length(a, b):
        if not a or not b:
            return 0
        # DP over the shorter dimension for lower memory
        if len(a) < len(b):
            short, long_ = a, b
        else:
            short, long_ = b, a
    ...
    for gen, gt in zip(gen_outputs, ground_truths):
        pred_tokens = _tokenize(gen or "")
        ref_tokens = _tokenize(gt or "")
        ...
        overlap = sum(min(ref_counts[t], pred_counts.get(t, 0)) for t in ref_counts)
        rouge1_recall.append(float(overlap) / float(len(ref_tokens)))

        lcs = _lcs_length(pred_tokens, ref_tokens)
        rougeL_recall.append(float(lcs) / float(len(ref_tokens)))
